...
function LeaderBoard() {
    ...
    Object.keys(leaderStats).sort((a, b) => (parseInt(leaderStats[b]["asked"]+parseInt(leaderStats[b]['voted']))) - (parseInt(leaderStats[a]["asked"]+ parseInt(leaderStats[a]['voted']))))

    let displayContent = <Spinner />
    if (leaderStats){
        displayContent = React.Children.toArray(Object.keys(leaderStats).map(key =>
            <SinglePerson author={key} asked={leaderStats[key]["asked"]} voted={leaderStats[key]["voted"]}/>
            ))
    }
